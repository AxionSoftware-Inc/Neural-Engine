"""Inference-only test of a post-GRU circuit correction residual for P-007."""
...
from benchmark_p007_correction_gain_sweep import (
    _accuracy,
    _loss,
    _shift_route,
)
...
@torch.no_grad()
def _evaluate_beta(model, batch, beta: float) -> dict[str, Any]:
    model.post_correction_residual_scale = float(beta)
    ...
    natural_ce = _loss(natural_logits, batch.targets)
    global_accuracy = _accuracy(global_logits, batch.targets)
    global_ce = _loss(global_logits, batch.targets)
    within_accuracy = _accuracy(within_logits, batch.targets)
    within_ce = _loss(within_logits, batch.targets)
    return {
        "beta": float(beta),
        "natural": {"accuracy": natural_accuracy, "ce": natural_ce},
        "global_route_replay": {
            "accuracy": global_accuracy,
            "ce": global_ce,
            "accuracy_delta_pp": (global_accuracy - natural_accuracy) * 100.0,
            "ce_increase": global_ce - natural_ce,
        },
        "within_task_route_replay": {
            "accuracy": within_accuracy,
            "ce": within_ce,
            "accuracy_delta_pp": (within_accuracy - natural_accuracy) * 100.0,
            "ce_increase": within_ce - natural_ce,
        },
    }


@torch.no_grad()
def evaluate_checkpoint(args: argparse.Namespace, path: Path) -> dict[str, Any]:
    payload = torch.load(path, map_location="cpu", weights_only=True)
    config = load_config(args.config, smoke=False) if args.config else dict(payload["config"])
    if config["model"] == "baseline":
        raise ValueError("post-correction residual requires a Neural Engine checkpoint")
    ...
    original_beta = float(model.post_correction_residual_scale)
    rows = [_evaluate_beta(model, batch, beta) for beta in args.betas]
    model.post_correction_residual_scale = original_beta
    return {
        "checkpoint": str(path),
        ...
        "checkpoint_post_correction_residual_scale": original_beta,
        "results": rows,
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="P-007 post-GRU correction residual sweep")
    ...
    parser.add_argument("--betas", type=float, nargs="+", default=[0.0, 0.125, 0.25, 0.5])
