...
// One block owns one token and visits its K selected groups in sequence.
// Unlike full_correction_kernel, this path accumulates the final hidden row in
// shared memory and writes it once, avoiding K atomic-add streams per token.
__global__ void token_full_correction_kernel(
    const float* __restrict__ hidden,
    const int64_t* __restrict__ selected_ids,
    const float* __restrict__ route_weights,
    const float* __restrict__ gate_weight,
    ...
    const int warp_count = (blockDim.x + 31) >> 5;
    float* warp_sums = latent + rank;
    float* output_vector = warp_sums + warp_count * rank;
    const int64_t output_base = token * hidden_size;
    for (int64_t h = threadIdx.x; h < hidden_size; h += blockDim.x) {
        output_vector[h] = 0.0f;
    }
    __syncthreads();

    const float* hidden_row = hidden + output_base;
    for (int64_t slot = 0; slot < active; ++slot) {
        ...
        for (int64_t h = threadIdx.x; h < hidden_size; h += blockDim.x) {
            ...
            output_vector[h] += scale * route * (selected_vector[h] + correction);
        }
        __syncthreads();
    }

    for (int64_t h = threadIdx.x; h < hidden_size; h += blockDim.x) {
        output[output_base + h] = output_vector[h];
    }
}
...
std::tuple<torch::Tensor, torch::Tensor> qwen_token_full_correction_dispatch_cuda(
    torch::Tensor hidden,
    torch::Tensor selected_ids,
    torch::Tensor route_weights,
    torch::Tensor gate_weight,
    ...
    auto selected = torch::empty({tokens, active, hidden_size}, hidden.options());
    auto output = torch::empty({tokens, hidden_size}, hidden.options());
    if (tokens == 0 || active == 0 || hidden_size == 0) {
        return std::make_tuple(selected, output);
    }
    const int threads = 256;
    const int warp_count = (threads + 31) / 32;
    const size_t shared_bytes = static_cast<size_t>(
        group_size + hidden_size + rank + warp_count * rank + hidden_size
    ) * sizeof(float);
    const auto stream = at::cuda::getCurrentCUDAStream();
    token_full_correction_kernel<<<
        static_cast<unsigned int>(tokens), threads, shared_bytes, stream
    >>>(
        hidden.data_ptr<float>(), selected_ids.data_ptr<int64_t>(),
        route_weights.data_ptr<float>(), gate_weight.data_ptr<float>(),
        value_weight.data_ptr<float>(), output_weight.data_ptr<float>(),
