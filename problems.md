...
`1..3` and holds out only depth `4` for 5,000 steps. Across seed17/18 the
held-out mean is `2.979%` versus V0.298's `14.355%`; fixed depth-4
add/subtract/multiply are `9.570%/8.984%/6.055%` versus
`36.621%/30.859%/4.199%`. The extra depth coverage therefore damages the
previously stronger add/subtract transfer and only adds `+1.855 pp` to
multiply. **REJECTED FOR ADOPTION AND SCALING; no 700M/1B scale.** The next
architecture target is an explicit operation-conditioned algebraic/value
