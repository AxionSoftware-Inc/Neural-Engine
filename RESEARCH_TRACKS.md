...
| Runtime | Dispatch, memory traffic, fused/compiled kernels used by either track | **Engineering track** | `exp/track-runtime` | Keep one worker per GPU baseline; keep admission opt-in until queue overhead beats direct serving under a latency budget; dynamic routing remains graph-unsafe |
