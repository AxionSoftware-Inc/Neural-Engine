...
| Runtime | Dispatch, memory traffic, fused/compiled kernels used by either track | **Engineering track** | `exp/track-runtime` | Use one worker per GPU as baseline; test batching/admission for shared-device throughput; dynamic routing remains graph-unsafe |
