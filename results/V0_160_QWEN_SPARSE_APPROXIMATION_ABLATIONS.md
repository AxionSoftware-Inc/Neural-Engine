...
the dense parent. Restricting replacement to late layers improves the error
relative to the full eight-layer cascade, but still fails the generalization
gate.

## Decision

These experiments reject three local repairs as sufficient solutions:
...
high-value path is a function-preserving compiled micro-group design with
explicit cross-group mixing or a learned low-rank shared basis. It should be
