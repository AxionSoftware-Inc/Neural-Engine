...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="include eager-only route-count bucketed grouped projections",
    )
    ...
    if args.include_grouped_adaptive_bucketed:
        path_specs.append((
            ...
            "grouped-adaptive-bucketed", True, True,
        ))
    ...
    grouped_adaptive_atomic_finalize_generation = None
    if args.include_grouped_adaptive_atomic_finalize:
        set_dispatch_path(
            children, False, "grouped-adaptive-atomic-finalize",
            uniform_accum=True,
        )
    ...
    grouped_adaptive_fixed_pack_generation = None
    if args.include_grouped_adaptive_fixed_pack:
        set_dispatch_path(
            children, False, "grouped-adaptive-fixed-pack",
            uniform_accum=True,
        )
    ...
    if args.include_grouped_adaptive_bucketed:
        set_dispatch_path(
            children, False, "grouped-adaptive-bucketed",
            uniform_accum=True,
        )
        grouped_adaptive_bucketed_generation = greedy_generate_fixed_shape(
            model, generation_prompt, 8, use_cuda_graph=True,
        )
    ...
    result = {
        ...
        "generation": {
            ...
            **({
                "grouped_vs_grouped_adaptive_atomic_finalize_exact_token_match": bool(
                    torch.equal(
                        grouped_generation,
                        grouped_adaptive_atomic_finalize_generation,
                    )
                ),
            ...
            **({
                "grouped_vs_grouped_adaptive_fixed_pack_exact_token_match": bool(
                    torch.equal(
                        grouped_generation,
                        grouped_adaptive_fixed_pack_generation,
                    )
                ),
            ...
            **({
                "grouped_vs_grouped_adaptive_bucketed_exact_token_match": bool(
                    torch.equal(
                        grouped_generation,
                        grouped_adaptive_bucketed_generation,
                    )
                ),
            } if grouped_adaptive_bucketed_generation is not None else {}),
        },
    }
